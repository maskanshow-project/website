  ...
  <el-card class="mb-3" shadow="hover" body-style="padding: 0px !important">
    <div class="feature-item row rtl list">
      <router-link class="col-md-4" :to=" 'estate/' + estate.id ">
        <div
          class="feature-pic js-tilt ltr bg-estate"
        ...
        >
          <div
            class="sale-notic"
            :style="{ background : estate.assignment.color + ' !important' }"
            v-if="!!estate.assignment && !!estate.estate_type"
          >{{ estate.assignment.title +' '+ estate.estate_type.title }}</div>
        ...
        <div class="assignmented" v-else-if="!estate.is_active">
          <div>
            <span class="stamp is-nope">تایید نشده</span>
          </div>
        </div>
      ...
      <router-link class="col-md-8" :to=" 'estate/' + estate.id ">
        ...
        <div class="feature-text mt-2">
          <div class="text-center feature-title row">
            <div class="col-md-7 pr-0">
              <router-link :to=" 'estate/' + estate.id ">
                <h5 class="bold">
                  <span
                    class="web-color"
                  >{{ '#'+ ( this.$route.params.username && estate.code ? estate.code : estate.id ) }}</span>
            ...
            <div
              class="col-md-5 border-right d-flex"
              :class="{ 'blur' : !!estate.assignmented_at || is_exist(estate.label) }"
            >
              ...
              <div class="pr-4">
                ...
                <p
                  class="fs-10 bold"
                  :class="estate.want_cooperation ? 'text-success' : 'text-danger'"
                ...
                <p
                  class="fs-10 text-muted bold"
                  v-else-if=" !( !!estate.assignmented_at || is_exist(estate.label) )
                                    && is_exist(estate.creator) && estate.creator.is_public_info "
                >{{ estate.creator.username+'@' }}</p>
            ...
            <div class="row room-info d-flex rtl">
              <template v-if="is_exist(estate.specifications)">
                ...
                >
                  <div
                    :class="set_grid"
                    v-if=" ( is_exist(spec.data) && spec.data != '[]' || spec.data == '0' ) || is_exist(spec.values) "
                    :key="idx+'spec'"
                  >
                    <p class="hvr-icon-back">
                      <i
                        :class="`fa fa-${ spec.row && spec.row.icon ? spec.row.icon : 'building' } hvr-icon web-color`"
                      ></i>
              ...
              <template v-if="is_exist(estate.detailable_features)">
                <template
                  v-for="(feature,idx) in estate.detailable_features.slice( 0 , slice_features )"
                >
            ...
            <div class="read-more rtl">
              <div class="theme-btn">
                <div v-if="assignment.has_sales_price">
                  ...
                  >{{ label_price(estate.sales_price) }}</span>
                </div>
                <div
                  class="d-flex"
                ...
                >
                  <div>
                    ...
                    <span
                      class="fs-12 normal"
                    >{{ label_price(estate.mortgage_price) }} رهن</span>
                  </div>
                  ...
                  <div>
                    ...
                    <span
                      class="fs-12 normal"
                    >{{ label_price(estate.rental_price) }} اجاره</span>
                  </div>
                </div>
                <div v-else>
                  ...
                  <span
                    class="fs-12 pr-1 normal"
                  >{{ label_price( assignment.has_mortgage_price ? estate.mortgage_price : estate.rental_price) }}</span>
                </div>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...mapState(["assignments", "url", "Auth", "req_url"]),

    assignment() {
      if (
        !_.isEmpty(this.assignments) &&
        !_.isEmpty(this.estate.assignment) &&
        !!this.estate.assignment.id
      ) {
    ...
    img() {
      return !_.isEmpty(this.estate.photos)
        ? { src: this.estate.photos[0].medium, has_img: true }
        : { src: "/img/default.jpg", has_img: false };
    },
    ...
    set_grid() {
      ...
      if (
        this.is_exist(this.estate.specifications) &&
        this.is_exist(this.estate.detailable_features)
      ) {
        length =
          this.estate.specifications.length +
          this.estate.detailable_features.length;
    ...
    slice_features() {
      let valid_spec = this.estate.specifications.filter(
        el =>
          (this.is_exist(el.data) && el.data != "[]") ||
          this.is_exist(el.values)
      );

      if (this.is_exist(this.estate.specifications)) {
        return valid_spec.length >= 8 ? 0 : 8 - valid_spec.length;
      } else {
        return this.estate.detailable_features.length >= 8
          ? 8
          : this.estate.detailable_features.length;
      }
    }
