...
export default new Vuex.Store({
  ...
  actions: {
    getData({ commit, state }, inputs) {
      ...
      axios.post("/graphql/auth", { query: query }).then(({ data }) => {
        commit("setData", {
          group: inputs.group,
          type: inputs.type,
          data:
            state[inputs.group].handleQuery &&
            state[inputs.group].handleQuery[inputs.type]
    ...
    getPermissions({ commit, state }, inputs) {
      ...
      axios
        ...
        .then(({ data }) => {
          ...
          if (
            data.data.me.allPermissions.filter(i => i.name === "read-comment")
              .length
          ) {
            axios
              .post("/graphql/auth", {
              ...
              .then(({ data }) =>
                commit("setInactiveComments", data.data.comments.total)
              );
          }
