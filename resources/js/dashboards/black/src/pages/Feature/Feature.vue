  ...
  >
        ...
        <tbody>
          ...
          >
            <td>
              <i
                class="tim-icons"
                :class="slotProps.row.is_detailable ? 'icon-check-2' : 'icon-simple-remove'"
              ></i>
            </td>
            <td>{{ slotProps.row.is_detailable ? 'نمایش' : 'عدم نمایش' }}</td>
          </tr>
        </tbody>
      </table>
    </template>
    ...
    <template #estate_types-body="slotProps">
      <transition-group name="list">
        ...
        <el-dropdown
          v-if="slotProps.row.estate_types.length > 3"
          :key="slotProps.row.estate_types.map((c) => c.id).join(',')"
        >
    ...
    <template slot="modal">
      ...
      <br />
      <md-chips v-model="similar_titles" md-placeholder="افزودن ...">
      ...
      <div class="row">
        <base-input class="w-100" label="وضعیت نمایش">
          ...
          >در صورت قابل نمایش بودن ، این آپشن بر روی کارت های املاک نمایش داده خواهد شد</small>
        </base-input>
      </div>
    </template>
  </datatable>
...
export default {
  ...
  mixins: [initDatatable, createMixin, validationMixin, Binding],
  metaInfo: {
    title: "امکانات املاک"
  },
  data() {
  ...
  computed: {
    title: bind("title"),
    similar_titles: bind("similar_titles"),
    ...
    allQuery() {
      return `title similar_titles description icon is_detailable estate_types { id title }`;
    }
  },
  mounted() {
