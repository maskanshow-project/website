<template>
  <div :style="{ position: 'relative', zIndex: 10 }">
    ...
    <card
      class="row ml-0 account-expire-timer animated bounceInRight delay-last"
      v-if="$store.state.me.validity_end_at"
    >
    ...
    <div class="row owl-carousel" v-if="articles && articles.length && false">
      <card v-for="article in articles" :key="article.id" class="text-right">
        <img
          slot="image"
          class="card-img-top"
          :src="article.image ? article.image.small : '/images/placeholder.png'"
          alt="article image"
        />
        <h4 class="card-title">
          {{ article.title | truncate(60) }}
          <span
            class="d-block badge badge-info mt-2 ml-1 hvr-grow-shadow hvr-icon-grow"
    ...
    <div class="row owl-carousel" v-if="plans && plans.length">
      <card
        v-for="plan in plans"
        :key="plan.id"
        class="plan-card text-right"
        :style="{ border: `1px solid ${plan.color}`, height: '380px' }"
      >
...
export default {
  ...
  mixins: [createMixin, filtersHelper],
  metaInfo: {
    title: "داشبورد"
  },
  data() {
  ...
  computed: {
    viewCredit() {
      if (!this.$store.state.me.plan.id) return 0;

      if (
        this.$store.state.me.remaining_hits_count >
        this.$store.state.me.plan.visited_estate_count
      )
        return 100;
      else
        return Math.round(
    ...
    registerCredit() {
      ...
      if (
        ...
        this.$store.state.me.plan.registered_estate_count
      )
        return 100;
      else
        return Math.round(
    ...
    expirationTime() {
      return (
        (moment(this.$store.state.me.validity_end_at) * 1 - moment() * 1) / 1000
      );
    }
  },
  methods: {
        ...
        axios
          ...
          .then(({ data }) => {
            ...
            this.final_price =
              ...
              (this.selected_plan.price * data.data.validatePromocode.count) /
                100;
  ...
  mounted() {
      ...
      setTimeout(() => {
        this.$swal
          .fire({
            ...
            icon: this.$route.query.message_type || "error",
            // timer: 3000,
    ...
    axios
      ...
      .then(({ data }) => {
        this.charts.chartData.labels = data.data.visitedEstates.map(
          i => i.month
        );
        this.charts.chartData.datasets[0].data = data.data.visitedEstates.map(
          i => i.count
        );
        this.charts.chartData.datasets[1].data = data.data.registeredEstates.map(
          i => i.count
        );
