    ...
    <div class="row">
      ...
      <div class="col-md-6">
        <card class="text-right animated bounceInBottom delay-last" v-if="is_loaded" dir="rtl">
          ...
          <div class="row" v-for="(opinion, index) in opinions" :key="index">
            <div class="col-md-12">
              <base-input label="تصویر شخص">
                ...
                <small slot="helperText" id="emailHelp" class="form-text text-muted">تصویر شخص صاحب نظر</small>
              </base-input>
            </div>
            <div class="col-md-12">
              <md-field>
          ...
          <div v-if="!opinions.length" class="alert alert-warning">
            متاسفانه بخش نظرات وبسایت در حال حاضر خالیست :(
          </div>

          <div class="operation-cell d-flex justify-content-center mt-3">
            <el-tooltip content="افزودن نظر جدید">
              <base-button @click="opinions.push({ avatar: {} })" class="hvr-icon-spin ml-2" type="success" size="sm" icon>
                <i class="tim-icons icon-simple-add hvr-icon" :style="{ fontSize: '18px !important' }"></i>
              </base-button>
            </el-tooltip>          
            
            <el-tooltip content="حذف آخرین نظر">
              <base-button @click="opinions.pop()" class="hvr-icon-spin ml-2" type="danger" size="sm" icon>
...
export default {
  ...
  mounted()
  {
    ...
    .then(({data}) => {
      ...
      this.opinions = data.data.siteSetting.opinions.map(i => {
        return { ...i, ...{ avatar: { url: i.avatar ? i.avatar.small : '', file: null } } }
      })
  ...
  methods:
  {
    addImage(type, file, index = null)
    {
      ...
      if ( type === 'poster' )
        this.posters[index].image = fileObj
      
      else if ( type === 'opinion' )
        this.opinions[index].avatar = fileObj
    ...
    updateSlider(type)
    {
      this.type = type === 'opinions' ? 'site_opinions' : 'site_posters'

      type === 'opinions' ? this.is_update_site_opinions = true : this.is_update_site_posters = true

      this.storeInServer({
        callback: ({data}) => {
          type === 'opinions' ? this.is_update_site_opinions = false : this.is_update_site_posters = false
        },
        errorResolver: (error) => {
          type === 'opinions' ? this.is_update_site_opinions = false : this.is_update_site_posters = false
        }
    ...
    changeFormData(fd)
    {
      ...
      {
        if ( this.opinions.filter(i => i.avatar.file).length === 0 )
          return fd;

        let map = {}

        this.opinions.forEach((opinion, index) => {
          if ( !opinion.avatar.file )
            return
