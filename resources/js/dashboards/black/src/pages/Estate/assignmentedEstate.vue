<template>
  ...
  >
    ...
    <template #address-body="slotProps">
      ...
      >
        <a
          ...
          slot="reference"
        >{{ slotProps.row.address + ' ، پلاک ' + slotProps.row.plaque | truncate(50) }}</a>
        <a
          v-else
          :href="`/estate/${slotProps.row.id}`"
          slot="reference"
      ...
      <table class="table-p-0">
        <tbody>
          ...
          <tr>
            ...
            <td class="text-right">
              <a
                :href="`telto:${slotProps.row.landlord_phone_number}`"
              >{{ slotProps.row.landlord_phone_number }}</a>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <template #assignment_count-body="slotProps">{{ slotProps.row.assignment_count }} دفعه</template>

    <template #custom-operations="slotProps">
      <el-tooltip content="تایید واگذاری">
        <base-button
          class="ml-2"
          @click="acceptAssignment(slotProps.index, slotProps.row)"
          type="warning"
          size="sm"
          icon
        >
  ...
  data() {
    return {
      ...
      emptyState: {
        ...
        description:
          "تمامی ملک ها تایید واگذاری شده اند ، در حال حاضر هیچ ملک جدیدی اعلام واگذاری نشده است"
      }
    };
  },
  methods: {
