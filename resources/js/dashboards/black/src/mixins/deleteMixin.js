...
export default {
  methods: {
    ...
    handleDelete(index, row) {
      this.$swal
        .fire({
          title: `برای پاک کردن ${this.label} "${
            row.name ? row.name : row.title ? row.title : "انتخاب شده"
          }" مطمئن هستید ؟`,
          ...
          if (result.value) {
            ...
            axios
              .post("/graphql/auth", {
                query: `mutation {
              ${mutation} (id: ${
                  this.attr("is_incrementing") ? `"${row.id}"` : row.id
                }) {
                status
                message
              }
    ...
    handleSelectionChange(items) {
      this.changeSelected({
        type: this.type,
        items: {
          items: (this.$store.state.feature.selected_items[
            this.type
            ...
            axios
              ...
              .then(({ data }) => {
                ...
                let newDataArray = this.$store.state[this.group][
                  ...
                  return (
                    !this.attr("selected_items").includes(index) ||
                    (this.ignoreOperations &&
                      this.ignoreOperations.includes(item.id))
                  );
                });

                this.setData(newDataArray);

                if (typeof this.afterMultipleDelete === "function")
                  this.afterMultipleDelete();
                ...
                this.$swal.fire({
                  ...
                  text: `${
                    this.label
                  } هایی که انتخاب کردید با موفقیت حذف شدند :)`,
                  icon: "success",
                  timer: 1000,
                  showConfirmButton: false
                });
