  ...
  <div>
    <transition name="fade" mode="out-in">
      <div v-show="attr('has_loaded')">
          ...
          <div class="row col-12">
            <div class="col-12 text-right">
              <div class="pull-right">
                ...
                >
                  مدیریت
                  <span :style="{ color: '#ff3d3d' }">{{ label }}</span> ها
                  <i
                ...
                >با استفاده از جدول زیر ، امکان مدیریت کامل {{ label }} های وبسایت برای شما ممکن خواهد شد</h6>
              </div>
          ...
          <slot name="charts">
            <div class="row details mb-3" :style="{ position: 'relative', zIndex: 2 }" dir="rtl">
              <div class="col-md-3">
                <div class="tilt">
                  <card
                    class="text-right mb-4 animated bounceInLeft delay-secound total-card"
                    :style="{ marginBottom: '32px !important', transformStyle: 'preserve-3d' }"
                  >
                    <i class="tim-icons icon-attach-87"></i>
                    <h5
                      class="card-category"
                      style="transform: translateZ(20px)"
                    >کل {{ label }} های موجود</h5>
                    ...
                    <p
                      ...
                      :style="{fontSize: '10px', transform: 'translateZ(15px)'}"
                    >تعداد {{ label }} هایی که تا کنون در وبسایت ثبت شده است</p>
                  </card>
                </div>
                <div class="tilt" :style="{ position: 'relative', top: '0px', right: '0px' }">
                  <card
                    class="text-right mb-0 animated bounceInLeft delay-last responsive trash-card"
                  ...
                  >
                    ...
                    <p
                      class="card-text text-muted"
                      :style="{fontSize: '10px', transform: 'translateZ(15px)'}"
                    >تعداد {{ label }} هایی که تا کنون حذف کرده اید</p>
                  </card>
                </div>
              </div>
        ...
        <div class="row table">
          <div class="w-100" :style="{ zIndex: 1000 }">
            ...
            <div class="animated bounceInLeft delay-secound">
              <input
                ...
                @keyup="search"
                dir="rtl"
                class="pull-right form-control col-3 ml-2 d-inline-block"
              ...
              <base-button
                ...
                v-if="canCreate"
                :disabled="abilities ? !abilities.create : can(`create-${type}`)"
                :type="abilities ? !abilities.create ? 'default' : 'success' : can(`create-${type}`) ? 'default' : 'success'"
                size="sm"
              >
                افزودن {{ label }} جدید
        ...
        >
          <template
            ...
            slot-scope="slotData"
          >
            <slot :name="`${item.field}-header`" v-bind="slotData" />
          </template>
          ...
          <template
            ...
            slot-scope="slotData"
          >
            <slot :name="`${item.field}-body`" v-bind="slotData" />
          </template>

          <template #custom-operations="slotProps">
            <slot name="custom-operations" :row="slotProps.row" :index="slotProps.index" />
          </template>
        </base-table>
        ...
        <md-dialog :md-active.sync="$store.state[group].is_open[type]" class="text-right" dir="rtl">
          <md-dialog-title>
            <h2 class="modal-title">{{ attr('is_creating') ? 'ثبت ' + label : 'ویرایش ' + label }}</h2>
          ...
          <md-dialog-actions>
            <base-button class="ml-2" type="danger" size="sm" @click="setAttr('is_open', false)">
...
export default {
  ...
  mixins: [deleteMixin, createMixin, FiltersHelper],
  data() {
    return {
  ...
  methods: {
    ...
    handleSelectAll(event) {
      let temp = event.target.checked
        ? Array.apply(null, { length: this.tableData().length }).map(
            (v, i) => i
          )
        : [];
    ...
    moreInfo(event) {
      event.target.parentNode.parentNode.parentNode.parentNode.classList.toggle(
        "active"
      );
    },
    lessInfo(event) {
    ...
    changeLayout(value) {
      if (value)
        $(".data-table")
          .removeClass(["animated", "bounceInUp", "delay-last"])
      ...
      setTimeout(() => {
        ...
        if (value)
          $(".data-table")
            ...
            .addClass(["animated", "bounceInUp", "delay-last"]);
        else
          $(".data-grid")
            .removeClass(["animated", "bounceOutDown"])
            .addClass(["animated", "bounceInUp", "delay-last"]);
    ...
    closePanel() {
      ...
      $(".navbar-brand")
        .removeClass(["animated", "fadeIn", "delay-1s"])
        .addClass(["animated", "fadeOut"]);
      $(".delay-first")
        .removeClass("delay-first")
        .addClass("delay-last-out");
      $(".delay-first")
        .removeClass("delay-secound")
        .addClass("delay-secound-out");
      $(".delay-last")
        .removeClass("delay-last")
        .addClass("delay-first-out");
      $(".bounceInUp")
    ...
    setAttr(attr, data, override = false) {
      let final_data =
        typeof data === "object"
          ? { ...this.$store.state[this.group][attr][this.type], ...data }
          : data;
...
.md-field .md-icon,
.md-field.md-has-textarea .md-icon {
...
.data-grid-item .card-footer,
.data-grid-item .card-header {
